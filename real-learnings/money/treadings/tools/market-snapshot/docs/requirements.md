...
Reason: locally computed values introduce model risk and bugs (wrong interest rate assumption, wrong time-to-expiry, wrong IV input). The broker's own risk system runs these calculations with exchange-grade inputs. Codex's only job is to interpret pre-calculated data and suggest strategies — not to verify math.

If a data field cannot be obtained pre-calculated from a trusted broker API, the field is stored as `null` in the snapshot. Codex is told to treat `null` fields as unavailable and not to estimate them.
...
|--------|-----------------|------|------|
| **Dhan MCP** (official, first-party) | Full option chain — all strikes, pre-calculated Greeks (Delta, Theta, Gamma, Vega), IV, OI, prev OI, volume, bid/ask. Called directly by Codex in conversation. | Dhan account; OAuth configured in user `~/.codex/config.toml` | **Preferred** |
...
```
Codex → Dhan MCP → Dhan servers → option chain + Greeks returned inline
Codex → NSE endpoint → VIX, spot, FII/DII, futures OI returned inline
```

No snapshot files needed. Codex gets live data on demand within the conversation.
...
Codex reads the file on request.
```
...
Every run creates a new file. No existing snapshot is ever modified or deleted. This enables Codex to track how OI, PCR, VIX, and Greeks changed across the session (OI vs Time, Fut OI vs Time) by comparing files in time order.

---

## What Each Snapshot Contains

**Zero processing rule:** the script calls the API, gets the response, and writes it directly to the JSON file. No field renaming, no restructuring, no type conversion, no derived values. What the API returns is what the file contains. Codex reads the raw API response.
...
### OI vs Time and Fut OI vs Time — derived by Codex, not by the script

Codex compares all `snapshot-*.json` files for the current day in time order. The script writes nothing extra for this. Codex reads raw `oi` and `previous_oi` values from the Dhan response across snapshots to track how OI moved over time per strike.

---

## How Codex Uses the Snapshots
...
With the full chain (all strikes, Greeks, OI) from Dhan, Codex can suggest:
...
Dhan has an **official first-party MCP server** (launched June 2026, docs at [docs.dhanhq.co/mcp](https://docs.dhanhq.co/mcp/)). It works with Codex exactly like the Kite and Kotak Neo MCPs already in `.codex/config.toml`.
...
|---------|---------------|
| **Dhan MCP exposes option chain + Greeks** | Add Dhan MCP to user `~/.codex/config.toml`. Codex calls it directly in conversation — real-time, no script, no snapshot files needed. The `market-snapshot` Python script is not built. |
