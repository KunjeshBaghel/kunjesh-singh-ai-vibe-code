...
Companion to [`broker-session-startup.md`](./broker-session-startup.md) (how to connect) and the capability map in [`AGENTS.md`](../AGENTS.md) (what each broker *claims* to support).

**Purpose of this file:** `AGENTS.md` records the *designed* architecture. This file records the **observed** reality — which endpoints actually respond, which are entitlement-blocked, and which MCP we ended up using for each data point in each session. Update it whenever a call behaves differently from the capability map.
...
| 01-Sep-2026 | 🟢 data only | 🟢 ₹7.02L, −₹15,564 loss | ⚠️ **MCP Unauthorized mid-session / REST ✅ all session** | Kite: NIFTY/VIX/OI/bid-ask (all working). Dhan MCP: `market_data_agent_tool` returned Unauthorized after 3 re-auth attempts — **OAuth session breaks mid-session**; REST API with `DHAN_ACCESS_TOKEN` worked reliably all day for full 231-strike chain. Kotak: positions + trade book. **Outcome: TRADE EXECUTED ❌ LOSS** — NIFTY 0-DTE expiry day. Bull Put Spread 24,000/23,800 PE. 9 lots short (avg ₹8.59), 10 lots long (₹1.60). NIFTY broke below 24,000 after 13:00; exit trigger (₹11.00 on 24,000 PE) fired at 13:11 but not executed. 9th lot ADDED at 13:11 @ ₹11.45 (wrong direction). Bought back at 14:51 @ ₹35.90. **Gross P&L: −₹15,564 (−2.22% of ₹7.02L).** Daily max loss limit (1.5% = ₹10,534) breached by ₹5,030. Root causes: (1) Structural error — used Bull Put Spread on a day where Pro desk had defined a ceiling via massive short calls (Bear Call Spread at 24,100 was the correct structure). (2) Missed exit trigger at 13:12. (3) Added 9th lot at trigger level instead of exiting. Full session: [`01-09-2026-tread.md`](../my-treads/September-2026/01-09-2026/01-09-2026-tread.md). **New Dhan finding:** MCP OAuth is unreliable mid-session — treat REST API as the primary path. **New KB rule — ⛔ SUPERSEDED 02-Sep-2026, see the row below.** As first written this rule read *"Pro desk net short calls >50K = CEILING"*. Both halves were wrong: it named **only Pro** (FII must be read first) and it set the threshold at **50K** (the hard ceiling is **80,000**; anything under 50,000 is *silence*, not permission for the opposite side). The live rule is **Gate 5** — six numbers, FII and Pro × net CE short / net PE short / futures. **New execution rule:** broker price alert must be set the moment Claude states a trigger price. |
...
| 02-Sep-2026 | 🟢 data (re-login needed at 09:16 — the 07:17 session did not survive to the open) | 🟡 ₹6,86,627 at 09:25, then **`Session Expired`** ~10:48 | 🔴 **MCP OAuth REGRESSED — Unauthorized ×2 (07:17, 09:27) / REST ✅ all session** | Kite: NIFTY/SENSEX/VIX spot, OI, `oi_day_high`, 15-min candles. Dhan **REST**: full chains for all 3 indexes, repeatedly, across the morning. Kotak: margin (one read only). **Outcome: NO TRADE — and the process failed before the market did.** A **Bull Put Spread was recommended against a Slightly Bearish classification** and was caught by the user, not by any gate. Corrected to SENSEX Bear Call 76,800/77,000 (credit 45.35 → 42.15 within 12 min; 76,700/76,900 at 52.60 was the better structure). NIFTY screened out empirically: the 100-wide condor decayed **1.1 pts in 75 minutes** at 4 DTE, and with credit > width/2 the k=1.5 stop sits beyond max loss — no working premium stop exists. Regime flipped twice inside an hour (kill switch 1/3 at 09:30 → 0/3 at 10:25 → bounce failing by 10:56). Full session: [`02-09-2026-tread.md`](../my-treads/September-2026/02-09-2026/02-09-2026-tread.md) · lessons: [`02-09-2026-learning.md`](../my-treads/September-2026/02-09-2026/02-09-2026-learning.md). | **New gate — Gate 5 (structure ↔ view ↔ participants).** The 01-Sep "Pro desk ceiling" rule inspected **only Pro**. On 01-Sep data Pro was net *long* 168,089 calls so it stayed silent — while **FII was net short 93,282 calls, above the 80,000 hard-ceiling threshold.** The rule was checking the wrong participant. Gate 5 now extracts **six** numbers (FII and Pro × net CE short / net PE short / futures), checks **FII first**, treats `<50,000` as **silence not permission**, and adds a hard forbid: ⛔ no Bull Put under any bearish view, ⛔ no Bear Call under any bullish view, no override. Also encoded: §8.11.7 must measure the **spread's intra-bar swing** (excluding the opening gap bar), the stop's **spot** level must be derived from chain `dC/dK` rather than eyeballed, and max profit / breakeven / max loss must be stated **before** any lot count. **Dhan:** `complete_login` replying `token_id already consumed` is **not** proof of binding — only `expirylist` is. One MCP attempt, then REST. |
| 03-Sep-2026 | 🟢 data only | 🟢 **₹6,86,627.79** (documented figure ₹7,02,275 is stale by ₹15,647 ≈ the 01-Sep loss) | 🟢 **MCP FIXED — root cause found** (see Notes) + REST ✅, both agree exactly | Kite: NIFTY/BANKNIFTY/SENSEX/VIX spot, `BSE:SENSEX` OHLC, 15/30-min index candles, **30-min option-leg candles for both spread legs** (tokens `219911941` / `219982853`). Dhan MCP **and** REST: expiry lists + full SENSEX 0-DTE chain, repeatedly. Kotak: `get_limits`. **Outcome: NO TRADE — reason code TOO THIN (§8.11.7, measured 0.92× vs a 1.5× floor).** SENSEX 03-Sep was the only index past Gate 1 (0-DTE; NIFTY 08-Sep = 4 sessions, BANKNIFTY 29-Sep monthly). Gate 2 ✅ (F = 76,731.7, four strikes within 1.6 pts, basis −8.8 pts = 0.011% of spot). Kill switch **0/3** — M1 no (10:30 bar wicked to 76,691.50 but *closed* inside the OR), **M2 not computable** (Kite returns `volume: 0` for cash indices → no VWAP; logged as a gap, **not scored green**), M3 no (77,000 CE +6.79M and 76,500 PE +6.29M `dOI` — two-sided writing, not directional). Gate 5 mandated **Bear Call**, forbade Bull Put. Gate 4 Go/No-Go scored **4 → SIT OUT** (RED on wall integrity: the only strike clearing both floors, 76,900, had already been traded through at 9:45; + 2 YELLOW). All six bear-call candidates priced: 76,900/77,000 (c/W 24.9%, 6 lots, ₹2,988) and 76,900/77,100 (21.7%, 3 lots, ₹2,604) cleared the money filters but stop at **76,911–76,915, inside the day's high of 76,924.48**; 77,000/77,100 made only **₹2,220** (< ₹2,500); 77,000/77,200 at **14.95%** and everything beyond at 11.4% or less failed c/W. Full session: [`03-09-2026-tread.md`](../my-treads/September-2026/03-09-2026/03-09-2026-tread.md) · lessons: [`03-09-2026-learning.md`](../my-treads/September-2026/03-09-2026/03-09-2026-learning.md). | **★ Dhan MCP `Unauthorized` was NEVER a consent problem — it is a STALE OAuth CLIENT REGISTRATION.** Four failures across 02–03 Sep were all diagnosed at the wrong layer. Fix: `claude mcp remove dhan -s local` → `claude mcp add --transport http --client-id "$DHAN_CLIENT_ID" dhan https://mcp.dhan.co/mcp` → user runs `/mcp` → open dhan → **Authenticate**. `mcp__dhan__login` was **not needed at all** afterwards. Diagnose by error string: `API Error: Unauthorized` = reached Dhan, rejected there → do the reset; `requires re-authorization (token expired)` = blocked locally by Claude Code OAuth → just `/mcp` → Authenticate. ⚠️ `claude mcp list` **✔ Connected** and `/mcp` showing `dhan · connected · 11 tools` are **transport-level only** and were green through all four failures. **★ New method — §8.11.7 measured on the real legs, not the index.** Pulling 30-min bars for both legs and pricing the spread at each bar's index extremes gave a **16.30-pt** intra-bar swing; the index-proxy eyeball gave ~11 pts (1.35×) vs the true **0.92×**. The rule says *the spread's own swing* and it means it. **★ New pre-strike test earned — the realised-vs-implied ratio.** Realised 232.98 pts in 98 min = **2.38 pts/min**; ATM-fwd straddle 295.25 ≈ **1.40 pts/min** for the 265 min remaining → **realised running ≈1.7× implied, i.e. VRP negative before friction.** That single ratio explains why *every* strike failed at once, and it is arithmetic on numbers already in hand — **run it right after Gate 2, before pricing strikes.** A low VIX is a low-*implied* signal, not a cheap-vol signal. **★ Also: score strikes against the day's HIGH/LOW, not spot** — measured from spot (76,744) the stops looked like comfortable 160–170 pt cushions. **Open — needs a ruling:** Gate 5's 80,000 threshold still does not say *daily change* or *cumulative net OI* (both agreed today); and the three-reason taxonomy misfits this session — "too thin" prescribes a structure change that did not exist, because the true cause was regime. Proposed 4th code: **"unpaid" (RV > IV)**. |
| 08-Sep-2026 | 🟢 data only | 🟢 ₹6,86,627.79 | 🟢 MCP ✅ (expirylist + optionchain, all 3 indexes) + REST ✅ | Kite: NIFTY/BANKNIFTY/SENSEX/VIX/sectoral spot + 5-day daily candles + 15-min intraday (08-Sep). Dhan MCP: `expirylist` all 3 indexes, `optionchain` NIFTY 08-Sep + SENSEX 10-Sep + BANKNIFTY 29-Sep. Dhan REST: NIFTY chain (correct `data.oc` structure confirmed). Kotak: `get_limits`. **Outcome: NO TRADE — `UNPAID` (Gate 1 vol-floor fails on all three indexes).** India VIX 11.21 < 13 (NIFTY + SENSEX). BANKNIFTY σ_ATM 10.10% < 16% (own straddle-implied, N=21 sessions to 29-Sep). VRP Form A NIFTY: realised/implied ratio 1.36 ≥ 1.0 → UNPAID confirmed. VRP Form B BANKNIFTY worst-window: 20.75%/10.10% = 2.05 ≥ 1.0 → UNPAID confirmed. Kill switch: M1 fires (two consecutive closes below OR low 23,668.15 at 11:45 and 12:00), M2 n/a (cash index, volume=0), M3 not checked (Gate 1 terminal) → 1/3. Gate 5: FII ΔPE +69,138 > 65,000 → Bear Call FORBIDDEN; Bull Put not forbidden but moot (Gate 1 fails). Note: 05-Sep observed market holiday (no NSE data); 09-Sep likely Muharram → NIFTY expiry advanced to Mon 08-Sep. NIFTY expired TODAY. Full session: [`08-09-2026-tread.md`](../my-treads/September-2026/08-09-2026/08-09-2026-tread.md). | **Dhan chain structure confirmed:** `json['data']['last_price']` = underlying spot; `json['data']['oc']` = strike dict keyed as floats (e.g. `"23650.000000"`) with `.ce` and `.pe` sub-dicts. Prior sessions used `json['data']` directly as the chain — incorrect; that only had two keys (`last_price`, `oc`). **BANKNIFTY σ_ATM calculation:** straddle ÷ (0.7979 × F × √(N/252)). For 29-Sep (N=21 sessions): F=57,147.60, straddle=1,328.90 → σ_ATM=10.10%. **Holiday pattern:** when NIFTY Tuesday expiry is on a public holiday, NSE advances it to Monday — always fetch expiry list (SI-8), never infer from the calendar. |
| 04-Sep-2026 | 🟢 data only | 🟢 ₹6,86,627.79 | 🟢 REST ✅ (expiry lists + full chains) | Kite: NIFTY/SENSEX/VIX/sectorals spot, 5-day daily candles (NIFTY token 256265, SENSEX token 265), 15-min intraday candles. Dhan REST: expiry lists (all 3 indexes), full option chains (NIFTY Sep-8 + SENSEX Sep-10). Kotak: `get_limits`. **Outcome: NO TRADE — `Too small` (Gate 1, sessions_to_expiry ≥ 3 on all indexes).** NIFTY nearest expiry Sep-8 = 3 sessions · SENSEX nearest Sep-10 = 5 sessions · BANKNIFTY Sep-29 monthly + locked. Gate 1 fires, Gates 2–5 not run. Kill switch 0/3 (M1 no closes outside 9:15–9:45 OR; M2 n/a cash index; M3 ungathered). VIX 10.97–11.01 (CHEAP). Strong global gap-up (S&P +1.06%, Nasdaq +1.40%, Hang Seng +2.08%). FII F&O T-1 strongly bearish — sold futures+calls, bought puts; 3-day bearish streak. Gate 5 SILENT (FII ΔCE +12,954 < 65K; Pro ΔCE +96,366 < 100K). NIFTY pinning zone 23,900–24,000; SENSEX 76,500–77,000. Next opportunity: NIFTY Sep-7 (Mon, 2 sessions = eligible); SENSEX Sep-9 (Tue, 2 sessions = eligible). Full session: [`04-09-2026-tread.md`](../my-treads/September-2026/04-09-2026/04-09-2026-tread.md). | **Friday structural pattern:** every Friday where Tuesday expiry is next week = NIFTY sessions_to_expiry = 3 → Gate 1 blocks. SENSEX (Thu expiry) is always 5 sessions from Friday → permanently blocked on Fridays. Tradeable Fridays are only the one immediately before NIFTY expiry (2 sessions). |
| 03-Sep-2026 **(post-close review, 15:34)** | 🔴 **session EXPIRED mid-session ~13:52** (`Please log in first`) — took no part in the review | 🟢 re-login via `get_login` → QR → `validate_login`; `get_limits` **₹6,86,627.79, unchanged** — nothing was deployed | 🟢 **carried the entire review alone** — chain, index candles, and per-leg `intraday` on `securityId` (`859031`/`859308` CE, `859073`/`859187` PE) | Dhan: full 0-DTE chain at 13:56, 15/60-min index candles, 15/60-min candles on four real option legs. Kotak: margin verify only. **Outcome: NO TRADE (`UNPAID`) — verdict UPHELD, but the day cost one expectancy-sized trade and exposed a rule defect.** **Close 76,152.86 — ON THE LOW.** −0.86%, range **771.62 pts = 2.2 × the 1 SD (357) estimated at 10:52**; the final 75 min fell **401 pts in a straight line, no upper wick**. **★ `TC §10b` was VINDICATED as a measurement:** it forecast realised/implied **1.71** at 11:05; the post-entry tape (11:00→15:30) delivered **1.72** (2.39 vs 1.39 pts/min). **⛔ A 13:56 mid-session re-score claiming the ratio was 0.97 and "biased toward UNPAID" is RETRACTED** — it was taken 90 min before the session's largest move. *`references/no-trade.md`'s "score at the hard flat, never at a mid-session snapshot" just proved itself inside our own review.* **Declined 76,900/77,000 Bear Call, 6 lots, credit 24.90:** MAE **31.30 vs a 39.84 stop** (buffer 57% 🟡, never 🟠), 12:30 time stop cleared at 31.5% capture, **50% target hit in the 13:30 bar → `TARGET`, +₹1,494 gross / ≈+₹1,344 net = 0.20%** — exactly book expectancy (₹1,274). Both legs settled 0.05. Full working: [`03-09-2026-tread.md` §15:34](../my-treads/September-2026/03-09-2026/03-09-2026-tread.md) · [`03-09-2026-learning.md`](../my-treads/September-2026/03-09-2026/03-09-2026-learning.md). | **★ THE FINDING — §10b is a SYMMETRIC test applied to an ASYMMETRIC structure.** The tape genuinely realised 1.7× implied, yet the Bear Call would have won large because **all the movement went away from the short strike**: adverse travel **53.07 pts** vs favourable **591.09 pts, ≈11:1**. `|range|` is the right test for someone short *both* wings; the only structure this book permits is **one-sided**, where movement away from the short strike is the payoff, not the risk. **The measurement window was never the defect — the symmetry is.** **★ THE COUNTERFACTUAL NOBODY PRICED.** A **76,500/76,400 Bull Put** at the 11:15 open: credit **27.15**, c/W **27.2%**, 7 lots, net credit **₹3,801** — it cleared c/W, the credit floor and stop reachability **more easily than any Bear Call did**, and it was on the wrong side of the day. At the **14:15 hard flat: +₹819. Held to expiry: −₹10,199** (≈ the full ₹10,500 structural cap). **The 2:15 PM SENSEX hard flat was worth ₹11,018 today on a trade that was never taken; the entire loss lived in the 75 minutes after it.** ⚠️ **And nothing in the rulebook would have blocked it** — the view was SIDEWAYS so §9's hard forbid never engaged, and under the new forbid-only Gate 5 semantics the participant read was SILENT. **Only the market view's one-line judgement "safer edge is the upside" pointed away, and that sentence is not a gate.** Do not take comfort from the +₹819: on a day the same move lands at 12:30 it is a max loss. **★ What called the day:** FII cumulative net LONG puts (−593,223) + net SHORT futures (−229,163), FII 5-day bearish / regime #2 Distribution-Trap, and breadth (BANK alone carrying). **★ What mis-called it:** PCR 1.087, max pain 76,800 (**close was 647 pts below it**), the "pin zone 76,700–76,900", and M3 reading two-sided OI as pin/range (**the 8.27M 76,500 put wall broke and closed 347 pts below**). **On 0-DTE in a trending tape, max pain is not a magnet and a wall is not a floor.** §10a's own expiry caveat — *treat PCR 0.90–1.10 as uninformative in the last two sessions* — applies exactly to 1.087 and **was not applied**; that is an existing rule being missed, not a new one needed. **⛔ NOTHING in `TRADING_CONSTANTS.md` changed today** (n=1, and n=1 in the favourable direction is the 31-Aug failure mode). **Month-end backtest items:** (1) §10b directional variant — count only realised travel *toward* the intended short strike; consider demoting `UNPAID` to a half-size warning for one-sided verticals while keeping it hard for two-sided; (2) close the SIDEWAYS→Bull-Put gap with a mechanical test, or stop treating the view line as a gate; (3) weight positioning over pin data in the five-view. **Dhan gotcha:** `intraday` **ignores `toDate`** — window a period by varying `interval` (60-min → 5–7 bars), not the date range; only the last 5 candles are ever printed. |
...
- [ ] 🔴 **NEW (28-Aug-2026) — fix or abandon the Dhan MCP OAuth binding** (§2.4). Dead 2 sessions running while REST works. Consider dropping the MCP from `.mcp.json` and documenting REST as the only Dhan path. **REOPENED 02-Sep-2026** — fixed on 31-Aug (§2.6), broken again by 02-Sep with **two `Unauthorized` failures in one morning** (07:17, 09:27) despite fresh consentIds. Raise alongside the Greeks defect above.
